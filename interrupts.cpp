...
/**
	Just a small function that prints interrupt number to the screen.
*/
void PrintInterruptNumber(uint8_t interrupt_number) {
	// Since interrupts are from 0-255 we only
	// print three digits.
	// 3 chars for digits, one for new line \n,
	// and one for null terminator \0 (added by compiler)
	char interrupt_number_string[5] = "000\n";
	// From least significant digit, to most
	// significant digit.
	interrupt_number_string[2] = '0' + interrupt_number % 10;
	interrupt_number /= 10;
	interrupt_number_string[1] = '0' + interrupt_number % 10;
	interrupt_number /= 10;
	interrupt_number_string[0] = '0' + interrupt_number % 10;
	printf("Interrupt number is: ");
	printf(interrupt_number_string);
}
uint32_t InterruptManager::HandleInterruptNonStatic(uint8_t interrupt_number, uint32_t esp) {
	PrintInterruptNumber(interrupt_number);
